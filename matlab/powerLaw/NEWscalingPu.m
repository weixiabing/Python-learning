...
for mi=M
    ...
    for b =ADCbit
        ...
        for ii=1:iter
            ...
            % WITH GROUP=K LINEAR ZF; WITH GROUP=1; ZF-SIC
            % imRicianMRCSEkntest( Pu, sigma2, K, mi, Hest, alpha, H, error);
...
% %preparing final results: starting from here as we need to modify the gross SE into net SE
...
% grid on;
%
% p(1)=plot(M,prelogFactor.*ZFSE(:,:,1),'-','LineWidth',2);
% p(2)=plot(M,prelogFactor.*GSICSEapp(:,:,1),'s','LineWidth',2);
% p(3)=plot(M,GSICSE(:,:,1),':','LineWidth',2);
% p(4)=plot(M,ZFSEapp(:,:,1),'s','LineWidth',2);
%
% p(5)=plot(M,prelogFactor.*ZFSE(:,:,2),'-','LineWidth',2);
% p(6)=plot(M,prelogFactor.*GSICSEapp(:,:,2),'s','LineWidth',2);
% p(7)=plot(M,GSICSE(:,:,2),':','LineWidth',2);
% p(8)=plot(M,ZFSEapp(:,:,2),'s','LineWidth',2);
%
% p(9)=plot(M,prelogFactor.*ZFSE(:,:,3),'-','LineWidth',2);
% p(10)=plot(M,prelogFactor.*GSICSEapp(:,:,3),'s','LineWidth',2);
% p(11)=plot(M,GSICSE(:,:,3),':','LineWidth',2);
% p(12)=plot(M,ZFSEapp(:,:,3),'s','LineWidth',2);
%
% legend( p([1 3  5 7 9 11 ]), '3-bit ADC with ZF' ,'3-bit ADC with ZFGSIC',...
%     '6-bit ADC with ZF','6-bit ADC with ZFGSIC','12-bit ADC with ZF','12-bit ADC with ZFGSIC');
%
