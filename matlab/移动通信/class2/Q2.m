...
hsl=[];%呼损率
dhl=[];
rmd1=[0.0050 0.0100 0.0150 0.0200 0.0250];
for rmd=rmd1;%呼叫到达速率
    ...
    while hjcs<1000
        ...
        % 掉话统计
        for n = 1:N
            if all([C0(n)==0, C1(n)==0, all(V0(n,:)==0), all(V1(n,:)==0), all(location0(n,:)==0), all(location1(n,:)==0)])
                dhcs = dhcs + 1;
            end
        end
...
%newloc函数是当数据点移出正六边形外后，该数据点在另一面所表示的坐标，
